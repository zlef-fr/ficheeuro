...
function load() {
  ...
  let elections = { byCirco: {} };
  try { elections = readJSON(path.join(DATA, "elections.json")); } catch {}
  let pays = { pays: [] };
  try { pays = readJSON(path.join(DATA, "pays.json")); } catch {}
  ...
  Object.assign(store, {
    ...
    // sigle → { lang: full group name }, from the EP's own list (empty if it was unavailable)
    groupesL10n: deputes.groupesL10n || {},
    stats,
...
// circo key "<dept>-<circo>" matching the Interior-ministry file
// (Français de l'étranger 099→ZZ, Saint-Barth/Martin 977→ZX).
function circoKey(circo) {
  if (!circo || !circo.numCirco) return null;
  let dep = (circo.numDepartement || "").trim();
  dep = /^\d+$/.test(dep) ? String(parseInt(dep, 10)) : dep;
  dep = ({ "99": "ZZ", "977": "ZX" })[dep] || dep;
  return dep + "-" + parseInt(circo.numCirco, 10);
}
...
function fiche(uid) {
  ...
  const ck = circoKey(f.circo);
  f.election = ck ? (store.elections.byCirco[ck] || null) : null;
  return f;
}
...
function search(q, limit = 30) {
  ...
  return store.deputes
    .map((d) => {
      // the national party is how most visitors know an MEP ("Reconquête", not "ESN")
      const hay = norm(`${d.prenom} ${d.nom} ${d.depNom} ${d.dep} ${d.groupe} ${d.groupeLibelle} ${d.parti || ""}`);
      let score = 0;
