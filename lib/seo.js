...
// Locale-specific share card: the default locale owns the bare /og path, every
// other locale renders its own card under /og/<lang>/ (built on the fly, cached).
function ogUrl(lang, slug) {
  return lang === L.DEFAULT ? `${BASE}/og/${slug}.png` : `${BASE}/og/${lang}/${slug}.png`;
}
...
function metaFor(routePath, lang) {
  ...
  if (fiche) {
    ...
    if (d) {
      ...
      return { ...m, routePath: `/depute/${d.slug}`, og: ogUrl(lang, d.slug), status: 200 };
    }
  }
  const dict = PAGES[lang] || PAGES[L.DEFAULT];
  const entry = dict[p] || PAGES[L.DEFAULT][p];
  if (entry) return { title: entry[0], desc: entry[1], routePath: p, og: ogUrl(lang, "default"), status: 200 };
  // Unknown route → a real not-found, not a soft-404 clone of the home page.
  const nf = ssr.notFoundText(lang);
  return { title: `${nf.h1} — ${B}`, desc: nf.lead, routePath: p, og: ogUrl(lang, "default"), status: 404 };
}
...
function personJsonLd(routePath, lang) {
  ...
  if (d.sexe === "F" || d.sexe === "H") ld.gender = d.sexe === "F" ? "Female" : "Male";
  // the party they were actually elected for, distinct from the EP group they sit in
  if (d.parti) ld.affiliation = [ld.affiliation, { "@type": "Organization", name: d.parti }];
...
// Returns { html, status } — status is 404 for unknown routes (a real not-found:
// noindex, no hreflang alternates, and a not-found body rather than the home clone).
