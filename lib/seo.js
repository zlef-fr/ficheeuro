...
const OG_DEFAULT = "https://fichedepute.eu/og.png";
const SUB = "Participation, votes et absences de votre eurodéputé au Parlement européen — en clair et 100 % sourcé.";
...
function metaFor(pathname) {
  ...
  if (fiche) {
    ...
    if (d) {
      ...
      return {
        title: `${name} (${d.groupe}) — FicheDéputé.eu`,
        desc: `${name}, eurodéputé·e ${circo}. ${d.presence}% de participation aux scrutins publics, votes, loyauté au groupe et activité législative — 100 % sourcé (Parlement européen).`,
  ...
  const pages = {
    "/deputes": ["Tous les eurodéputés — FicheDéputé.eu", "Cherchez et comparez les 719 eurodéputés du Parlement européen (2024-2029) : participation, votes, groupe et circonscription."],
    "/classements": ["Classements des eurodéputés — FicheDéputé.eu", "Les plus assidus, les plus absents, les plus actifs, les plus loyaux et les plus frondeurs du Parlement européen."],
    "/groupes": ["Les groupes politiques — FicheDéputé.eu", "Participation moyenne et effectifs de chaque groupe politique du Parlement européen."],
    "/jeu": ["Devine l'eurodéputé — FicheDéputé.eu", "Le jeu civique : présence + 3 votes POUR et 3 votes CONTRE, à toi de deviner de quelle fiche il s'agit."],
    "/methode": ["Méthode & sources — FicheDéputé.eu", "D'où viennent les chiffres de FicheDéputé.eu : les jeux open data officiels du Parlement européen (licence Ouverte)."],
  };
  if (pages[p]) return { title: pages[p][0], desc: pages[p][1], canonical: `${BASE}${p}`, og: OG_DEFAULT };
  return { title: "FicheDéputé.eu — La fiche vivante de votre eurodéputé·e", desc: SUB, canonical: `${BASE}/`, og: OG_DEFAULT };
}

function attr(s) { return String(s).replace(/[<>&"]/g, (c) => ({ "<": "&lt;", ">": "&gt;", "&": "&amp;", '"': "&quot;" }[c])); }

// FAQPage JSON-LD on the Méthode route so crawlers & AI lift the answers (esp.
// "présence non publiée nominativement") without running the SPA. FR canonical.
