...
function metaFor(routePath, lang) {
  ...
  if (fiche) {
    ...
    if (d) {
      ...
      return { ...m, routePath: `/depute/${d.slug}`, og: ogUrl(lang, d.slug) };
    }
  }
  const dict = PAGES[lang] || PAGES[L.DEFAULT];
  const entry = dict[p] || PAGES[L.DEFAULT][p];
  if (entry) return { title: entry[0], desc: entry[1], routePath: p, og: ogUrl(lang, "default") };
  const home = dict["/"];
  return { title: home[0], desc: home[1], routePath: "/", og: ogUrl(lang, "default") };
}
